...
static spx_profiler_reporter_cost_t trace_notify(spx_profiler_reporter_t *base_reporter,
...
static void print_row(spx_output_stream_t *output, const char *prefix,
                      const spx_php_function_t *function, size_t depth, const int *enabled_metrics,
...
static void flush_buffer(trace_reporter_t *reporter, const int *enabled_metrics)
{
    ...
    for (i = 0; i < reporter->buffer_size; i++) {
        ...
        print_row(
            reporter->output, entry->event_type == SPX_PROFILER_EVENT_CALL_START ? "+" : "-",
            entry->function, entry->depth, enabled_metrics, &entry->cum_metric_values,
                      ...
                      const spx_profiler_metric_values_t *exc_metric_values)
{
    ...
    SPX_METRIC_FOREACH(i, {
        ...
        spx_fmt_row_add_ncell(fmt_row, 1, spx_metric_info[i].type, cum_metric_values->values[i]);

        const double inc = inc_metric_values ? inc_metric_values->values[i] : 0;
        spx_fmt_row_add_ncell(fmt_row, 1, spx_metric_info[i].type, inc);

        const double exc = exc_metric_values ? exc_metric_values->values[i] : 0;
        spx_fmt_row_add_ncell(fmt_row, 1, spx_metric_info[i].type, exc);
    });
    ...
    snprintf(func_name, sizeof(func_name), format, prefix, function->class_name,
             function->class_name[0] ? "::" : "", function->func_name);
