...
static const spx_time_provider_t linux_time_provider = {.get_wall_time_ns = linux_get_wall_time_ns,
...
static size_t parse_status_value(const char *buf, size_t buf_len, const char *key)
{
    ...
    while (i < buf_len) {
        if (buf[i] == ':') {
            ...
            if (i - line_start == key_len && strncmp(buf + line_start, key, key_len) == 0) {
...
static int linux_memory_get_stats(spx_memory_stats_t *stats, spx_error_t *error)
{
    ...
    if (bytes_read < 0) {
        SPX_ERROR_SET(error, SPX_ERR_FILE_READ_FAILED, "Failed to read status: %s",
                      strerror(errno));
        return -1;
    }
...
static const spx_memory_provider_t linux_memory_provider = {.get_stats = linux_memory_get_stats,
...
static int linux_io_get_stats(spx_io_stats_t *stats, spx_error_t *error)
{
    ...
    if (bytes_read < 0) {
        SPX_ERROR_SET(error, SPX_ERR_FILE_READ_FAILED, "Failed to read I/O stats: %s",
                      strerror(errno));
        return -1;
    }
    buf[bytes_read] = '\0';
    ...
    /* Parse: rchar: <value>\nwchar: <value>\n... */
    size_t *targets[] = {&stats->bytes_read, &stats->bytes_written, &stats->read_ops,
                         &stats->write_ops};
    int target_idx = 0;
    const char *p = buf;
...
static const spx_io_provider_t linux_io_provider = {.get_stats = linux_io_get_stats,
...
static int linux_fs_get_info(const char *path, spx_file_info_t *info, int follow_symlinks,
                             spx_error_t *error)
{
    if (!path || !info) {
...
static int linux_fs_realpath(const char *path, char *resolved, size_t size, spx_error_t *error)
{
...
static const spx_fs_provider_t linux_fs_provider = {
    .get_info = linux_fs_get_info, .mkdir = linux_fs_mkdir, .realpath = linux_fs_realpath};
...
static const spx_platform_info_t linux_platform_info = {
    ...
    .capabilities = SPX_PLATFORM_CAP_WALL_TIME | SPX_PLATFORM_CAP_CPU_TIME |
                    ...
                    SPX_PLATFORM_CAP_PROCFS | SPX_PLATFORM_CAP_TTY | SPX_PLATFORM_CAP_ANSI_COLORS,
    .name = "Linux",
    .version = NULL /* Could read /proc/version */
};

static const spx_platform_t linux_platform = {.info = &linux_platform_info,
...
int spx_platform_init(spx_error_t *error)
{
    ...
    if (linux_ctx.status_fd < 0) {
        SPX_ERROR_SET(error, SPX_ERR_FILE_NOT_FOUND, "Cannot open /proc/self/status: %s",
                      strerror(errno));
